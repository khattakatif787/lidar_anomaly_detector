...
def load_pcd_fields(pcd_path: str) -> Dict[str, Optional[np.ndarray]]:
    """
    Loads required fields from PCD supporting DATA ascii and DATA binary.
    Returns dict with:
      - xyz: (N,3) float32
      - intensity: (N,) float32 or None if missing
    """
...
def _polar_entropy(xyz: np.ndarray, n_az: int = 32, n_r: int = 16) -> float:
    """
    Optional: 2D polar-grid entropy in (azimuth x range) bins.
    Measures distribution "disorder" (object-agnostic).
    """
...
def extract_features_corruption_plus(
    ...
    Enriched XYZ-only features.
    Output dims:
      37 if include_entropy=False
      38 if include_entropy=True
    """
    if xyz is None or xyz.shape[0] == 0:
...
def extract_features_from_pcd(pcd_path: str, feature_set: str = "corruption_plus") -> np.ndarray:
    ...
    xyz = d["xyz"]
    intensity = None  # (we ignore intensity; corrupted PCDs often don't have it)
